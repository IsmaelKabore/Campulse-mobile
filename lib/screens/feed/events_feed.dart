...
class _EventsFeedState extends State<EventsFeed> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<PostProvider>().loadPostsByCategory(PostCategory.events);
    });
  }
          ...
          SliverPadding(
            ...
            sliver: Consumer<PostProvider>(
              builder: (context, postProvider, _) {
                if (postProvider.isLoading && postProvider.events.isEmpty) {
                  return const SliverFillRemaining(
                    child: AppLoadingState(),
                  );
                }

                if (postProvider.error != null && postProvider.events.isEmpty) {
                ...
                if (postProvider.events.isEmpty) {
                  return const SliverFillRemaining(
                    child: AppEmptyState(
                ...
                // Grid layout matching web: grid-cols-1 gap-7 md:grid-cols-2 lg:grid-cols-3
                return SliverGrid(
                  ...
                  delegate: SliverChildBuilderDelegate(
                    (context, index) {
                      final post = postProvider.events[index];
                      return EventCard(
                        post: post,
                        onTap: () {
                          Navigator.push(
                        ...
                        likeCount: 0, // TODO: Get from post
                      );
                    },
                    childCount: postProvider.events.length,
                  ),
                );
              },
            ),
